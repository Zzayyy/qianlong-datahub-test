...
# （需 create 按行顺序发送）。ref_seq=当天全局起始号（当日已有 ref_seq-1 张时传入）。


def build_bulk_rows(count, start=0, ref_seq=1):
    """生成 count 行 normal：每行一个不同账号，修改与 create 行序对齐的全局单号。"""
    count = int(count)
    if count <= 0:
    ...
    for i in range(count):
        ...
        r = list(template)
        r[idx["case_no"]] = f"MGi{i + 1:04d}"
        r[idx["case_type"]] = "normal"
        r[idx["case_desc"]] = f"账号{facct} 修改单号{ref_seq + i}"
        r[idx["FAccount"]] = facct
        r[idx["Ref"]] = f"__REF{ref_seq + i}__"
        out.append(tuple(r))
    return out
